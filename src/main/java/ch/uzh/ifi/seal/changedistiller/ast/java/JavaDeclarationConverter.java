...
public class JavaDeclarationConverter extends ASTVisitor {
	...
	public boolean visit(Argument node, BlockScope scope) {
		...
		if (isNotParam) {visitModifiers(node.modifiers, node.declarationSourceStart, node.modifiersSourceStart)
			;
		}
...
//		fScanner.resetTo(fieldDeclaration.declarationSourceStart, fieldDeclaration.sourceStart());
		visitModifiers(fieldDeclaration.modifiers, fieldDeclaration.declarationSourceStart, fieldDeclaration.sourceStart());
	}
...
//		fScanner.resetTo(methodDeclaration.declarationSourceStart, methodDeclaration.sourceStart());
		visitModifiers(methodDeclaration.modifiers, methodDeclaration.declarationSourceStart, methodDeclaration.sourceStart());
	}
...
//		fScanner.resetTo(typeDeclaration.declarationSourceStart, typeDeclaration.sourceStart());
		visitModifiers(typeDeclaration.modifiers, typeDeclaration.declarationSourceStart, typeDeclaration.sourceStart());
	}

	static String[] modifierArray = { "abstract", "public", "protected", "private", "final", "static", "volatile",
			"strictfp", "native", "synchronized", "transient" };

	// logic partly taken from org.eclipse.jdt.core.dom.ASTConverter
	private void visitModifiers(int modifierMask, int aSourceStart, int aDeclarationStart) {
		push(JavaEntityType.MODIFIERS, "", -1, -1);
		if (modifierMask != 0) {
			Node modifiers = fNodeStack.peek();
//			int aBodyStart = fSource.indexOf("{");
//			if (aBodyStart == -1) {
//				return;
//			}
			String aHeader = fSource.substring(aSourceStart, aDeclarationStart);
			for (String aModifier : modifierArray) {
				int anIndex = aHeader.indexOf(aModifier);
				if (anIndex < 0) {
					continue;
				}
				int anEndIndex = anIndex + aModifier.length() - 1;
				push(JavaEntityType.MODIFIER, aModifier, anIndex, anEndIndex);
				pop();
			}
	...
	// logic partly taken from org.eclipse.jdt.core.dom.ASTConverter
	private void visitModifiersBuggy(int modifierMask) {
		push(JavaEntityType.MODIFIERS, "", -1, -1);
		if (modifierMask != 0) {
			Node modifiers = fNodeStack.peek();
	...
	public boolean visit(TypeParameter typeParameter, BlockScope scope) {
		push(fASTHelper.convertNode(typeParameter),
				getSource(typeParameter.sourceStart(), typeParameter.declarationSourceEnd), typeParameter.sourceStart(),
